...
export class OpenAIGenerationService extends BaseGenerationService {
   ...
   */
  private static async saveRecipeToDatabase(recipe: Recipe): Promise<Recipe | null> {
    try {
      ...
      // Save to database
      const { data, error } = await supabase.from('recipes').upsert([dbRecipe]);
  ...
  private static async saveRecipesToDatabase(recipes: Recipe[]): Promise<Recipe[]> {
    try {
      ...
      // Save to database
      const { data, error } = await supabase.from('recipes').upsert(dbRecipes);
