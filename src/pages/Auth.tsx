...
import { ChefHat, LogInIcon, UserPlusIcon, MailIcon, LockIcon } from 'lucide-react';
...
const Auth = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      await withPerformanceTracking('User signup process', async () => {
        ...
        // Use admin client to create profile with elevated permissions
        const client = getClient(true);
        
        // Then, create the profile - fix the types to match what the database expects
        const { error: profileError } = await client.from('profiles').insert({
          id: signUpData.user.id,
          email: signUpData.user.email || '', // Ensure email is always a string
        ...
        if (profileError) {
          ...
          handleSupabaseError(profileError, 'profile creation');
          ...
          >
            ...
            <div className="text-center mb-6">
              <div className="flex justify-center mb-2">
                <ChefHat className="h-12 w-12 text-kitchen-500" />
              </div>
