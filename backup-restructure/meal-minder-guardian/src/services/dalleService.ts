...
const DEPLOYED_FUNCTION_URL = 'https://meal-minder-guardian.functions.supabase.co/dalle-image-gen';
...
console.log(`[DALL-E Service] Current origin: ${window.location.origin}`);
...
export const testDalleService = async (): Promise<string> => {
  ...
  try {
    // Always use debug mode for test calls to avoid using real API credits
    ...
    if (!response.ok) {
      ...
      console.error(`[DALL-E] Test failed: ${response.status} - ${errorText}`);
      return FALLBACK_IMAGES.burger;
    }
    ...
    try {
      ...
      if (!data.imageUrl) {
        console.error('[DALL-E] No image URL in test response');
        return FALLBACK_IMAGES.burger;
      }
      ...
      console.error('[DALL-E] Error parsing response:', parseError);
      return FALLBACK_IMAGES.burger;
    }
  } catch (error) {
      ...
      const doRequest = async (): Promise<string> => {
        for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
          try {
            ...
            // Always use debug mode to avoid using real API credits
            const response = await fetch(DALLE_FUNCTION_URL, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
            ...
            try {
              ...
              // Check if there was an error but the function provided a fallback image
              if (data.error) {
                console.warn(`[DALL-E] Server reported error but provided fallback: ${data.error}`);
              }
...
export const generateRecipeImage = async (recipe: Recipe): Promise<string> => {
  ...
  try {
    // Get a fallback image right away so we have it if needed
    ...
    // Try to use the DALL-E function
    try {
      const imageUrl = await generateImage(prompt);

      // Check if we got a fallback SVG - if so, use our nice Unsplash fallback instead
      if (imageUrl === SVG_FALLBACK_IMAGE) {
        console.log(
          `[DALL-E] Got SVG fallback, using nice image fallback for recipe: ${recipe.title}`
        );
